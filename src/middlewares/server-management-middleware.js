...
function validateServerManagementRequest(req, res, next) {
    ...
    if (!req.is('application/json')) {
        ErrorResponse.message = 'Something went wrong while creating Gateway Management';
        ...
        ErrorResponse.message = 'Something went wrong while creating Gateway Management';
        ErrorResponse.error = new AppError(['gateway_ip not found in the incoming request in the correct form'], StatusCodes.BAD_REQUEST);
        return res
            .status(StatusCodes.BAD_REQUEST)
            .json(ErrorResponse);
    }
        ...
        ErrorResponse.message = 'Something went wrong while creating Gateway Management';
        ErrorResponse.error = new AppError(['gateway_name not found in the incoming request in the correct form'], StatusCodes.BAD_REQUEST);
        return res
            .status(StatusCodes.BAD_REQUEST)
            .json(ErrorResponse);
    }
...
function modifyServerManagementUpdateBodyRequest(req, res, next) {
    ...
    try {
        ...
        ErrorResponse.message = 'Something went wrong while updating Gateway.';
        ErrorResponse.error = error;
        return res
            .status(StatusCodes.INTERNAL_SERVER_ERROR)
            .json(ErrorResponse);
...
function validateDeleteRequest (req, res, next) {
    ...
    if (!req.is('application/json')) {
        ErrorResponse.message = 'Something went wrong while Deleting Gateway';
        ErrorResponse.error = new AppError(['Invalid content type, incoming request must be in application/json format'], StatusCodes.BAD_REQUEST);
        return res
            .status(StatusCodes.BAD_REQUEST)
            .json(ErrorResponse);
