...
async function updateCredit(req, res) {
  ...
  try {
    ...
    if (fromUpdatedUser?.dataValues?.id === (parentUser?.dataValues?.id)) {
      ...
      let errorMessage = "";
      if (fromUpdatedUser?.dataValues?.role == USERS_ROLE.SUPER_ADMIN) {
        errorMessage = "Superadmin cannot directly recharge other Users";
      } else if (fromUpdatedUser?.dataValues?.role == USERS_ROLE.RESELLER) {
        errorMessage = "Reseller cannot directly recharge other Users";
      } else if (fromUpdatedUser?.dataValues?.role == USERS_ROLE.COMPANY_ADMIN) {
...
async function updateCompanyCredit(req, res) {
  const bodyReq = req.body;
  ...
  try {
    ...
    let user;
    let fromUser;
    const updatedUser = await companyRepo.findOne({ id: bodyReq.id });
    const parentUser = await userRepo.get(updatedUser?.dataValues?.created_by);
    const fromUpdatedUser = await userRepo.get(bodyReq.fromUser);
    if (fromUpdatedUser?.dataValues?.id === (parentUser?.dataValues?.id)) {
      if (userRole == USERS_ROLE.SUPER_ADMIN) {
      ...
      Logger.info(
        `Credit Company -> Updated successfully: ${JSON.stringify(responseData)}`
      );
      return res.status(StatusCodes.ACCEPTED).json(SuccessRespnose);
    } else {
      let errorMessage = "";
      if (fromUpdatedUser?.dataValues?.role == USERS_ROLE.SUPER_ADMIN) {
        errorMessage = "Superadmin cannot directly recharge other Companies";
      } else if (fromUpdatedUser?.dataValues?.role == USERS_ROLE.RESELLER) {
        errorMessage = "Reseller cannot directly recharge other Companies";
      } else if (fromUpdatedUser?.dataValues?.role == USERS_ROLE.COMPANY_ADMIN) {
        errorMessage = "Company User cannot directly recharge other Companies";
      } else {
        errorMessage = "You cannot directly recharge other Companies";
      }
      return res
        .status(StatusCodes.BAD_REQUEST)
        .json({ message: errorMessage });
    ...
    Logger.error(
      `Credit -> unable to update Company Credit: ${JSON.stringify(
        bodyReq
      )} error: ${JSON.stringify(error)}`
    );
